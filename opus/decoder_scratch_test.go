...
// internal/celt/scratch.go as a black-box regression test; under -tags poison the
// same test runs with every reused window stamped 0x5A, which is the strongest
// form. The 24 kHz case decodes with downsample=2, reaching the deemphasis
// scratch that 48 kHz decoding never allocates.
	...
	for _, ch := range []int{1, 2} {
		for _, rate := range []int{48000, 24000} {
			t.Run(fmt.Sprintf("ch%d/%dHz", ch, rate), func(t *testing.T) {
				...
				// A second stream with a different frame size and different content,
				// used only to leave its values behind in the reused decoder's pool.
				...
				for i := range dirtyPkts {
					buf := make([]byte, 1500)
					n, err := dirtyEnc.Encode(encTestPCM(480, ch, 48000, i+100), buf)
					if err != nil {
				...
				dirtyOut := make([]int16, rate/100*ch)
